...
CREATE TABLE PLANTSHOP (
	...
	CONSTRAINT PK_PLANTSHOP_NO PRIMARY KEY(PARCELNO),
    CONSTRAINT FK_PLANTSHOP_WRITER FOREIGN KEY(mNO) REFERENCES MEMBER(mNO) ON DELETE SET NULL
);    
INSERT INTO PLANTSHOP VALUES(0, 1, '선인장 분양합니다', '애지중지 2년동안 키운 선인장 분양합니다', '식물', '원본파일명.txt', '변경된파일명.txt', DEFAULT, DEFAULT, 5000, 'Y', DEFAULT);
COMMIT;
...
-- 검색용 쿼리 3 종 세트
-- 리스트용 쿼리 완성
SELECT  P.PARCELNO, P.PARCELTITLE, M.ID, P.CRTDATE, P.PARCELIMG, P.PARCELCOUNT, P.STATUS
FROM PLANTSHOP P JOIN MEMBER M ON(P.PARCELNO = M.mNO)
WHERE 1 = 1 
AND P.STATUS = 'Y'
-- AND M.ID LIKE '%admin%' 
-- AND B.TITLE LIKE '%구매%' 
 AND P.CONTENT LIKE '%선인장%' 
...
SELECT  P.PARCELNO, P.PARCELTITLE, M.ID, P.PARCELCOUNT, P.PARCELIMG, P.PARCELIMGEDT, P.PARCELCONTENT, P.CRTDATE, P.MDFDATE,
