						...
						<div class="accordion accordion-alt pb-2 mb-4" id="shopCategories">
									...
									<div class="accordion-body py-1 mb-1">
										<div class="form-check">
											...
											<label class="form-check-label d-flex align-items-center"
												for="clay-pot"> <span class="text-nav fw-medium">토분</span></label>
										</div>
										<div class="form-check">
											<input class="form-check-input" type="checkbox" id="ceramics"
												name="kinds" value="도자기"
												${fn:contains(paramMap.kinds, '도자기') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="ceramics"> <span class="text-nav fw-medium">도자기</span></label>
										</div>
										<div class="form-check">
											<input class="form-check-input" type="checkbox" id="plastic"
												name="kinds" value="플라스틱"
												${fn:contains(paramMap.kinds, '플라스틱') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="plastic"> <span class="text-nav fw-medium">플라스틱</span></label>
										</div>
										<div class="form-check">
											<input class="form-check-input" type="checkbox" id="cement"
												name="kinds" value="시멘트"
												${fn:contains(paramMap.kinds, '시멘트') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="cement"> <span class="text-nav fw-medium">시멘트</span></label>
										</div>
									</div>
								</div>
							</div>
							<div class="accordion-item mb-0">
								...
								<div class="accordion-collapse collapse" id="care"
									...
									<div class="accordion-body py-1 mb-1">
										<div class="form-check">
											<input class="form-check-input" type="checkbox" id="trowel"
												name="kinds" value="모종삽"
												${fn:contains(paramMap.kinds, '모종삽') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="care-all"><span class="text-nav fw-medium">모종삽</span></label>
										</div>
										<div class="form-check">
											<input class="form-check-input" type="checkbox"
												id="Nutrients" name="kinds" value="영양제"
												${fn:contains(paramMap.kinds, '영양제') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="repotting"><span class="text-nav fw-medium">영양제</span></label>
										</div>
										<div class="form-check">
											<input class="form-check-input" type="checkbox"
												id="Pesticide" name="kinds" value="살충제"
												${fn:contains(paramMap.kinds, '살충제') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="water"><span class="text-nav fw-medium">살충제</span></label>
										</div>
										<div class="form-check">
											<input class="form-check-input" type="checkbox" id="soil"
												name="kinds" value="흙"
												${fn:contains(paramMap.kinds, '흙') ? 'checked':'' }>
											<label class="form-check-label d-flex align-items-center"
												for="temp"><span class="text-nav fw-medium">흙</span></label>
										</div>
									</div>
								</div>
							</div>
					...
					<c:forEach var="item" items="${list}">
						<div class="col pb-2 pb-sm-3">
							...
							<div class="d-flex mb-1">
								<h3 class="h6 mb-0">
									<div id="title" name="title" type="text" placeholder="Search">
										<a href="${path}/shop/product?pno=${item.pno}"
											style="text-decoration-line: none; color: black;">${item.title}</a>
									</div>
								</h3>
								...
								<c:forEach begin="${pageInfo.startPage}"
									...
									<c:if test="${status.current != pageInfo.currentPage}">
										<li class="page-item"><a class="page-link"
											onclick="movePage(${status.current});"
